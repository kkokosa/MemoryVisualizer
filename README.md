...
**Implemented now:** the [#6](https://github.com/kkokosa/MemoryVisualizer/issues/6) foundation, [#7](https://github.com/kkokosa/MemoryVisualizer/issues/7) bounded worker protocol, and [#8](https://github.com/kkokosa/MemoryVisualizer/issues/8) native dump snapshot adapter and CLI summary. Snapshots include memory maps, reference edges, roots, and handle metadata. The Electron shell and v1 worker protocol still use their explicit **fake backend**, not the new adapter. **Not implemented:** MQL, graph storage, root-path algorithms, layout/rendering, exports, packaging, or the desktop workspace UX. The old WPF/FsXaml application and Neo4j/Java/Paket startup dependencies have been removed rather than ported.
...
| --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/MemoryVisualizer.Core`             | Platform-neutral snapshot/runtime-scoped identities, materialized memory-map/reference/root/handle records, DTO conversion, and additive analysis interfaces. No ClrMD, native, UI, JSON framework, or Electron dependency.        |
